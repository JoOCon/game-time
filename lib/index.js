...
const jumpOnLog = () => {
  if (logs.find(log => log.y + 2 === frog.y && frog.x >= log.x && frog.x <= log.x + log.width - 55)) {
    frog.onLog = true;
  } else {
    frog.onLog = false;
  }
...
const drift = () => {
  const logType = logs.find(log => log.y + 2 === frog.y);
  if (frog.onLog) {
    frog.x += logType.direction == 'right' ? (logType.speed) : (logType.speed * -1);
  }
};
