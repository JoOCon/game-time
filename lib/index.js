...
const gameLoop = () => {
  const evenLogs = logs.evens;
  const oddLogs = logs.odds;
  const evensXDirection = -1;
  const oddsXDirection = 1;

  ctx.clearRect(0, 0, canvas.width, canvas.height);
  background();
  evenLogs.forEach(log => {
    log.move(evensXDirection).draw(ctx);
  });
  oddLogs.forEach(log => {
    log.move(oddsXDirection).draw(ctx);
  });
