...
class Game {
  ...
  drown() {
    if (this.frog.y < 350 && this.frog.y > 50 && !this.frog.floating) {
      --this.lives;
      this.reset();
    }
  }
  ...
  hasMadeItToLillyPad() {
    ...
    // ctx.fillRect(250, 0, 100, 50);
    // ctx.fillRect(425, 0, 100, 50);
    if (this.frog.y < 50 && this.frog.x >= 75 && this.frog.x <= 175 || this.frog.y < 50 && this.frog.x >= 250 && this.frog.x <= 350 || this.frog.y < 50 && this.frog.x >= 425 && this.frog.x <= 525) {
      ++this.wins;
      this.reset();
    }
  }
