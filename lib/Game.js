...
class Game {
  constructor(frogImg, logImg, carImg, truckImg) {
    ...
    this.vehicles = this.generateAutos(this.carImg, this.truckImg);
  }
  ...
  //make the frog able to be killed by the autos hitting him.
  vehicleHitFrog() {
      this.vehicles.forEach(vehicle => {
  ...
  drown() {
    if (this.frog.y < 350 && !this.frog.floating) {
      this.lives--;
      this.reset();
    }
  }
  ...
  hasMadeItToLillyPad() {
    if (this.frog.y < 50 && this.frog.x >= 75 && this.frog.x <= 175 ||
      this.frog.y < 50 && this.frog.x >= 250 && this.frog.x <= 350 ||
      this.frog.y < 50 && this.frog.x >= 425 && this.frog.x <= 525) {
      this.wins++;
      this.reset();
    }
  }
