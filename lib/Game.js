...
class Game {
  ...
  generateLogs() {
    // for loop to create a new log generation
  ...
  generateAutos() {
    // for loop to create a new auto generation
  ...
  drawLogs(ctx) {
    this.logs.forEach(log => log.move().draw(ctx));
  }

  drawVehicles(ctx) {
    this.vehicles.forEach(vehicle => vehicle.move().draw(ctx));
  }

  vehicleHitFrog() {
    this.vehicles.forEach(vehicle => {
      // break out conditions in to variables
      if (vehicle.x < this.frog.x + this.frog.width &&
         vehicle.x + vehicle.width > this.frog.x &&
         vehicle.y < this.frog.y + this.frog.height &&
          vehicle.height + vehicle.y > this.frog.y) {
        this.lives--;
        this.reset();
      }
    });
  ...
  drown() {
    if (this.frog.y < 350 && !this.frog.y <= 0 && !this.frog.floating) {
      this.lives--;
      this.reset();
    }
  }
